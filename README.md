# Nathan Toner - Cybersecurity Portfolio

## About Me
I earned my Bachelor of Science in Computer and Information Systems from Robert Morris University with a focus in Data Analytics. I have developed hands-on cybersecurity experience through labs, technical projects, coursework, and security-focused exercises.

My experience includes network security, Linux security, vulnerability assessment, security operations, governance, risk and compliance (GRC), incident response, TLS and certificate security, and data analytics. I enjoy identifying security weaknesses, analyzing technical information, solving problems, and developing practical solutions to improve security.

##Education
Robert Morris University
Bachelor of Science in Computer and Information Systems
Focus: Data Analytics
Completed 2026

My education provided experience in data analysis, information systems, programming, databases, cybersecurity, and technical problem solving.
## Goals
I am working toward a career in cybersecurity and want to continue developing my technical and analytical skills through hands-on experience. My goals are to build experience in security operations, governance, risk and compliance (GRC), incident response, vulnerability management, and other areas of cybersecurity while continuing to expand my knowledge and certifications.

As I gain professional experience, I would also like to grow into a leadership position where I can help guide security teams, improve security practices, and contribute to an organization's overall security strategy.
## Skills at a Glance

| Category | Tools / Concepts |
|---|---|
| Networking & Security Fundamentals | Network Security, Nmap, Wireshark, Metasploit, Vulnerability Assessment, Firewalls |
| Linux & Windows Administration | Linux, Ubuntu, Windows Server, Active Directory, File Permissions, Security Hardening |
| SOC Tools & SIEM | Microsoft Sentinel, Microsoft Defender, Security Monitoring, Detection Rules, Incident Response |
| Cryptography & PKI | OpenSSL, TLS, Digital Certificates, Encryption, Hashing, Authentication |
| GRC | NIST CSF, Governance, Risk and Compliance, Security Controls, Policy and Compliance |
| Data Analytics | Power BI, Python, Pandas, Matplotlib, SQL, MySQL |

## What's In This Repository

| Folder | What You'll Find |
|---|---|
| [soc-tools](./soc-tools) | SOC tools, security monitoring, SIEM exercises, and incident response labs |
| [malware-analysis](./malware-analysis) | Malware analysis labs, research, and documented findings |
| [grc](./grc) | Governance, risk, compliance, security frameworks, and control analysis |
| [research-and-writeups](./research-and-writeups) | Cybersecurity research, technical writeups, and lab findings |
| [presentations](./presentations) | Cybersecurity presentations and course materials |
| [templates](./templates) | Reusable cybersecurity templates and documentation |
| [certifications-and-skills](./certifications-and-skills) | Certifications, training, and skills developed throughout the bootcamp |
| [tls-audits](./tls-audits) | TLS, certificates, encryption, PKI, and security audit work |

## QR Code
![QR code linking to this project](github-qr-code.png)
## Website

Live version of this portfolio: https://ntoner.github.io/CybersecurityforProfessionals/

## Contact

Email: ntoner1346@gmail.com

GitHub: https://github.com/ntoner

LinkedIn: https://www.linkedin.com/in/nathan-toner-b86424354?utm_source=share_via&utm_content=profile&utm_medium=member_ios
